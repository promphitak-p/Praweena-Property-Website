...
document.addEventListener('DOMContentLoaded', async () => {
  // 1) ลองอ่านพิกัดจาก data-attribute บน container (เผื่อดึงมาจากหน้า list)
  const mapEl = document.getElementById('map');
  const noGeo = document.getElementById('no-geo');

  // ตัวอย่าง: ใส่ data-lat/lng บน #map ถ้ามี (เช่นจาก server-render)
  ...
  // 2) ถ้าไม่มี lat/lng ให้ลองดึงจาก Supabase ด้วย slug หรือ property_id ใน query string
  let finalLat = lat, finalLng = lng;
  if (!finalLat || !finalLng) {
    const params = new URLSearchParams(location.search);
    const pid = params.get('id') || params.get('slug'); // ปรับให้ตรงกับของจริง
    if (pid) {
      // โหลดจาก Supabase
      ...
      if (supabaseUrl && supabaseKey && createClient) {
        ...
        const { data, error } = await sb
          ...
          .single();
        if (!error && data) {
          finalLat = data.lat; finalLng = data.lng;
        }
      }
    }
  }
  ...
  if (finalLat && finalLng) {
    mapEl.hidden = false;
    const map = L.map('map').setView([finalLat, finalLng], 16);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; OpenStreetMap'
    }).addTo(map);
    L.marker([finalLat, finalLng]).addTo(map);
  } else {
    noGeo.hidden = false;
  }
});
