...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function searchAndRender() {
    ...
    let query = sb.from('properties').select('id,slug,title,desc,price,beds,cover').eq('published', true);

    if (q) query = query.ilike('search_text', `%${q}%`); // ทำ materialized 'search_text' รวมชื่อ+ทำเล
    ...
    if (error) { box.innerHTML = `<div class="notice">เกิดข้อผิดพลาด: ${error.message}</div>`; return; }
    box.innerHTML = data.map(p => `
      <a class="card" href="/property-detail.html?id=${p.slug || p.id}">
        <img src="${p.cover || '/assets/placeholder.jpg'}" alt="">
        <div class="card-body">
          <h3>${p.title}</h3>
          <p>${p.desc ?? ''}</p>
          <div class="meta">฿${Number(p.price).toLocaleString()} • ${p.beds} ห้องนอน</div>
        </div>
      </a>
    `).join('') || `<div class="notice">ไม่พบรายการที่ตรงเงื่อนไข</div>`;
  }

  document.getElementById('search-form').addEventListener('submit', (e) => { e.preventDefault(); searchAndRender(); });
  searchAndRender();
});
</script>
